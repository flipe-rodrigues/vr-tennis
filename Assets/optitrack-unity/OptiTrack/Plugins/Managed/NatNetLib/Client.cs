...
#pragma warning restore CA1031 // Do not catch general exception types
    ...
    internal class NatNetServerDiscovery : IDisposable
    {
        ...
        private static Dictionary<IntPtr, NatNetServerDiscovery> s_discoveryInstances = new Dictionary<IntPtr, NatNetServerDiscovery>();
        private static object s_discoveryInstancesLock = new object();
        private static NatNetServerDiscoveryCallback s_staticDiscoveryCallback;
        #endregion Private fields

        static NatNetServerDiscovery()
        {
            s_staticDiscoveryCallback = StaticServerDiscoveredNativeThunk;
        }
        ...
        public NatNetServerDiscovery(IEnumerable<string> knownServerAddresses = null)
        {
            if (knownServerAddresses == null)
            {
                NatNetError result = NatNetLib.NativeMethods.NatNet_CreateAsyncServerDiscovery(
                    out m_discoveryHandle,
                    s_staticDiscoveryCallback,
                    IntPtr.Zero);
            ...
            lock (s_discoveryInstancesLock)
            {
                s_discoveryInstances[m_discoveryHandle] = this;
            }
        }

        private static void StaticServerDiscoveredNativeThunk(sNatNetDiscoveredServer discoveredServer, IntPtr pUserContext)
        {
            try
            {
                NatNetServerDiscovery instance = null;

                lock (s_discoveryInstancesLock)
                {
                    // Try to find instance by iterating (since we don't have the handle in the callback)
                    foreach (var kvp in s_discoveryInstances)
                    {
                        instance = kvp.Value;
                        break; // Use first available instance (for single-instance scenarios)
                    }
                }

                if (instance != null && !instance.m_disposed)
                {
                    instance.ServerDiscoveredHandler(discoveredServer);
                }
            }
#pragma warning disable CA1031 // Do not catch general exception types
            catch (Exception ex)
#pragma warning restore CA1031 // Do not catch general exception types
            {
                System.Diagnostics.Debug.WriteLine("ERROR - Exception occurred in StaticServerDiscoveredNativeThunk: " + ex.ToString());
            }
        }

        private void ServerDiscoveredHandler(sNatNetDiscoveredServer discoveredServer)
        {
            lock (m_discoveredServers)
            {
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            lock (s_discoveryInstancesLock)
            {
                s_discoveryInstances.Remove(m_discoveryHandle);
            }
